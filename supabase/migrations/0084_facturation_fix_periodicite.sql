-- =====================================================================
-- 0084 — Facturation : correctif génération forfaits (périodicité)
--
-- Bug : un forfait de périodicité 'unitaire' (ou non reconnue) produisait un
-- pas de génération nul → erreur « step size cannot equal zero » qui faisait
-- échouer toute la fonction (aucune facture générée, 0 € partout).
-- Correctif : les forfaits 'installation' ET 'unitaire' (ou autre) sont
-- facturés en one-shot (une seule fois, à la date de début).
-- À exécuter après 0083.
-- =====================================================================
...
begin
  ...
  -- 3) Forfaits récurrents : une facture par période écoulée (bornée fin de PEC).
  for fo in
  ...
  loop
    ...
    -- pas de génération : récurrent si journalier/hebdo/mensuel ; sinon one-shot.
    ...
    for d in
      select g::date from generate_series(
        fo.date_debut,
        case when v_step is null then fo.date_debut else v_fin end,
        coalesce(v_step, interval '1000 years')
      ) g
      where g::date <= v_fin
    loop
