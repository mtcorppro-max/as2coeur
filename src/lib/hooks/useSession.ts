...
export type SessionPro = { id: string; nom: string; prenom: string | null; titre: string | null; role: string; niveau: number; agence_id: string | null; prestataire_id: string; user_id: string };

const LS_PATIENT = "sc_patient";
const LS_PRO = "sc_pro4"; // bump : ajout agence_id
...
async function fetchPro(): Promise<SessionPro | null> {
  ...
  const { data } = await supabase
    .from("professionnel")
    .select("id,nom,prenom,titre,role,niveau,agence_id,prestataire_id,user_id")
    .eq("user_id", session.user.id)
    .maybeSingle();
  if (!data) return null;
