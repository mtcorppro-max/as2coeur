...
type Eq = { id: string; numero_serie: string; chez_patient_depuis: string | null; type: { nom: string } | { nom: string }[] | null };
const nomType = (v: Eq["type"]) => (Array.isArray(v) ? v[0]?.nom : v?.nom) ?? "Équipement";
...
export function EquipementsPatient({ patientId }: { patientId: string }) {
  ...
  useEffect(() => {
    createClient()
      .from("equipement")
      .select("id,numero_serie,chez_patient_depuis,type:type_id(nom)")
      ...
      <div className="grid grid-cols-1 gap-1.5">
        {eqs.map((e) => (
          <div key={e.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-rose-100 px-3 py-1.5 text-sm">
            <span className="text-slate-700">{nomType(e.type)} · <span className="font-mono text-slate-500">{e.numero_serie}</span></span>
